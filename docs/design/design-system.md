...
| modal ([1475:3406](https://www.figma.com/design/T2upW8lruemIBSEwLZ6Lpc/web_%ED%94%84%EB%A0%99_%EB%94%94%EC%9E%90%EC%9D%B8-%EC%9E%91%EC%97%85%EB%B0%A9?node-id=1475-3406&m=dev)) | `shared/ui/modal` | 제목+설명+보조/권장 액션 2버튼 확인 다이얼로그. 액션 버튼은 형제 버튼과 폭을 균등하게 나눠 갖는(`flex-1`) 레이아웃이라 `shared/ui/button`의 고정폭/고정높이 사이즈와 안 맞아, 이 컴포넌트 안에서 Figma 색·크기 값을 그대로 쓰는 raw `<button>`으로 둠. `document.body`로 포탈 렌더링, Esc/오버레이 클릭은 `onDismiss`로 위임. `useConfirmModal` 훅으로 `await confirm({...})` 형태의 선언적 확인창 API도 함께 제공 |
