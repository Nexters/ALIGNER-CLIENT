...
2. 현재 브랜치명에서 이슈 번호를 파싱한다 (`/start-work`가 만드는 브랜치는 `feat/issue-<번호>` 또는 `fix/issue-<번호>` 형태). 번호를 못 찾으면 사용자에게 물어본다.
3. 커밋되지 않은 변경이 있으면 사용자에게 먼저 확인한다. 원격에 브랜치가 없으면 push한다.
4. `.github/pull_request_template.md`의 섹션 구조를 그대로 채워서 PR을 생성한다. `gh pr create`는 non-interactive 모드에서 템플릿을 자동으로 불러오지 않으므로, 아래처럼 heredoc으로 직접 채운 body를 넘긴다:

```bash
gh pr create --assignee @me --title "<변경 요약>" --body "$(cat <<'EOF'
## Summary
