...
import { StepChromeProvider } from "./step-chrome-context";
...
export function Layout({ onComplete }: LayoutProps) {
  return (
    <StepChromeProvider>
      <LayoutContent onComplete={onComplete} />
    </StepChromeProvider>
  );
}

function LayoutContent({ onComplete }: LayoutProps) {
  ...
  return (
    <main className="relative flex w-full min-h-screen flex-col justify-start bg-tertiary-50 px-[2rem]">
