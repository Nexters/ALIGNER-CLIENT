...
| 세션 (Session) | 이 레포에서 "세션"은 항상 운동 세션(시작~완료 단위)을 가리킨다. 로그인/인증 여부를 가리킬 때는 "인증(Auth)"이라 부르고 "세션"이라 하지 않는다 | `entities/session`(운동, 추후)과 `shared/api`의 인증 상태(`isAuthenticated`/`setAccessToken`)로 분리해 이름 충돌을 피한다 |
